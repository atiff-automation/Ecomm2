...
export default function DeliveryTimelineVisualization({
  ...
  const timelineData = useMemo(() => {
    const currentStageIndex = DELIVERY_STAGES.findIndex(stage => 
      stage.key === currentStatus || 
      (currentStatus === 'COMPLETED' && stage.key === 'DELIVERED')
    );
    
    const progressPercentage = progress || (currentStageIndex >= 0 ? 
      ((currentStageIndex + 1) / DELIVERY_STAGES.length) * 100 : 0);
    ...
    return {
      ...
      stages: DELIVERY_STAGES.map((stage, index) => ({
        ...
        hasEvent: trackingEvents.some(event => 
          event.eventCode === stage.key || 
          (stage.key === 'IN_TRANSIT' && ['IN_TRANSIT', 'ARRIVED_AT_HUB', 'DEPARTED_FROM_HUB'].includes(event.eventCode))
        ),
      })),
    };
  ...
  return (
    <div className={`space-y-6 ${className}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
              ...
              {actualDelivery && (
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="font-medium">{formatDate(actualDelivery, true)}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <CardContent>
          <div className="space-y-6">
            {timelineData.stages.map((stage, index) => {
              ...
              const stageEvents = trackingEvents.filter(event => 
                event.eventCode === stage.key ||
                (stage.key === 'IN_TRANSIT' && ['IN_TRANSIT', 'ARRIVED_AT_HUB', 'DEPARTED_FROM_HUB'].includes(event.eventCode))
              );
                    ...
                    {stageEvents.length > 0 && (
                      <div className="mt-2 space-y-1">
                        {stageEvents.map((event, eventIndex) => (
                          <div key={eventIndex} className="text-xs text-gray-500 ml-2 border-l-2 border-gray-200 pl-2">
                            ...
                            {event.courierRemarks && (
                              <p className="text-gray-600 italic">"{event.courierRemarks}"</p>
                            )}
                          </div>
                        ))}
                      </div>
      ...
      {/* Event Categories */}
      {Object.entries(categorizedEvents).some(([_, events]) => events.length > 0) && (
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
