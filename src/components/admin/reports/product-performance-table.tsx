...
export function ProductPerformanceTable({ 
  ...
  const fetchProductPerformance = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/reports/sales/products?${params}`);
      ...
      if (result.success) {
        ...
        throw new Error(result.message || 'Failed to fetch product performance');
      }
    } catch (error) {
  ...
  if (products.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <h3 className="text-lg font-medium text-muted-foreground">No product data found</h3>
          ...
          <TableBody>
            {products.map((product, index) => (
              <TableRow key={product.productId}>
                ...
                <TableCell className={`text-right font-medium ${getProfitColor(product.profitMargin)}`}>
                  ...
                  <div className="flex items-center justify-center space-x-2">
                    <div className="text-center">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {getMemberPercentage(product.memberSales, product.totalRevenue).toFixed(0)}% Member
                      </div>
                    </div>
                    ...
                    <div className="text-center">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {(100 - getMemberPercentage(product.memberSales, product.totalRevenue)).toFixed(0)}% Non-Member
                      </div>
                    </div>
                  </div>
                </TableCell>
