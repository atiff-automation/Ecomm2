...
import { activateUserMembership, getMembershipConfig } from '@/lib/membership';
import { z } from 'zod';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const products = await prisma.product.findMany({
      where: {
        id: { in: orderData.cartItems.map(item => item.productId) },
        status: 'ACTIVE',
      },
      include: {
        category: {
    ...
    for (const cartItem of orderData.cartItems) {
      ...
      if (!product.isPromotional && product.category?.isQualifyingCategory) {
