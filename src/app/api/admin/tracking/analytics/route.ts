...
export async function GET(request: NextRequest) {
  try {
    ...
    const deliveredShipments = shipments.filter(
      s => s.actualDelivery && s.createdAt
    );
    ...
    const courierStats = shipments.reduce(
      (acc, shipment) => {
        ...
        if (shipment.status === 'DELIVERED') {
          ...
          if (shipment.actualDelivery && shipment.createdAt) {
            const deliveryTime =
              new Date(shipment.actualDelivery).getTime() -
              new Date(shipment.createdAt).getTime();
            acc[courierName].totalDeliveryTime +=
              deliveryTime / (1000 * 60 * 60 * 24);
    ...
    const courierPerformance = Object.entries(courierStats)
      .map(([courierName, stats]) => ({
        courierName,
        shipmentCount: stats.shipmentCount,
        deliveryRate:
          stats.shipmentCount > 0
            ? (stats.delivered / stats.shipmentCount) * 100
            : 0,
        averageTime:
          stats.deliveredWithTime > 0
            ? stats.totalDeliveryTime / stats.deliveredWithTime
            : 0,
      }))
