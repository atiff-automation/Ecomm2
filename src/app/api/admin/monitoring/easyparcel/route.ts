...
const monitoringActionSchema = z.object({
  action: z.enum(['get_stats', 'get_alerts', 'resolve_alert', 'export_metrics', 'test_alert', 'clear_metrics']),
  timeRange: z.number().min(60000).max(7 * 24 * 60 * 60 * 1000).optional(), // 1 minute to 7 days
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { action, timeRange = 60 * 60 * 1000, alertId, severity } = monitoringActionSchema.parse(body);

    const monitor = EasyParcelMonitor.getInstance();

    console.log(`[EasyParcel Monitoring] ${action} action requested by ${session.user.email}`);
...
async function clearOldMetrics(cutoffTime: number): Promise<number> {
  try {
    ...
    console.log(`[EasyParcel Monitoring] Clearing metrics older than ${new Date(cutoffTime).toISOString()}`);
...
function calculatePerformanceTrends(metrics: any[]): any {
  ...
  const firstHalfErrorRate = firstHalf.filter(m => !m.success).length / firstHalf.length;
  const secondHalfErrorRate = secondHalf.filter(m => !m.success).length / secondHalf.length;
  ...
  const errorRateTrend = secondHalfErrorRate > firstHalfErrorRate * 1.1 ? 'increasing' : 
                        secondHalfErrorRate < firstHalfErrorRate * 0.9 ? 'decreasing' : 'stable';

  return {
    responseTime: responseTimeTrend,
    errorRate: errorRateTrend,
    throughput: firstHalf.length < secondHalf.length ? 'increasing' : 
...
function generatePerformanceAnalysis(stats: any, trends: any): any {
  ...
  if (stats.errorRate > 0.10) {
    ...
    analysis.concerns.push(`High error rate: ${(stats.errorRate * 100).toFixed(1)}%`);
    analysis.recommendations.push('Investigate and fix failing API calls');
  } else if (stats.errorRate > 0.05) {
    analysis.overall = 'fair';
    analysis.concerns.push(`Elevated error rate: ${(stats.errorRate * 100).toFixed(1)}%`);
  }

  // Analyze response time
  if (stats.averageResponseTime > 5000) {
    analysis.overall = 'poor';
    analysis.concerns.push(`Slow response time: ${stats.averageResponseTime.toFixed(0)}ms`);
    ...
    analysis.concerns.push(`Moderate response time: ${stats.averageResponseTime.toFixed(0)}ms`);
  }

  // Analyze trends
  if (trends.responseTime === 'increasing') {
    analysis.concerns.push('Response times are trending upward');
    analysis.recommendations.push('Monitor performance and investigate bottlenecks');
  }
  ...
  Object.entries(stats.performance).forEach(([operation, perfStats]: [string, any]) => {
    if (perfStats.successRate < 0.90) {
      analysis.concerns.push(`${operation} has low success rate: ${(perfStats.successRate * 100).toFixed(1)}%`);
    }
    if (perfStats.averageDuration > 3000) {
      analysis.concerns.push(`${operation} is slow: ${perfStats.averageDuration.toFixed(0)}ms`);
    }
  });
