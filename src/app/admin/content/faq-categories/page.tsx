/**
 * Admin FAQ Category Management Page
...
import type { FAQCategoryWithFAQCount } from '@/types/faq-category.types';
...
export default function AdminFAQCategoryListPage() {
  const router = useRouter();

  // State
  const [categories, setCategories] = useState<FAQCategoryWithFAQCount[]>([]);
  ...
  const fetchCategories = async () => {
    try {
      ...
      const response = await fetch(`/api/admin/faq-categories?${params}`);
  ...
  // Handle delete
  const handleDelete = async (id: string, name: string, faqCount: number) => {
    if (faqCount > 0) {
      toast.error(`Cannot delete category with ${faqCount} FAQ(s). Please reassign or delete the FAQs first.`);
      return;
    }
  ...
  // Handle reorder
  const handleReorder = async (reorderedCategories: FAQCategoryWithFAQCount[]) => {
    try {
      const payload = prepareReorderPayload(reorderedCategories);

      const response = await fetchWithCSRF(DRAG_DROP_CONSTANTS.API_ROUTES.REORDER_CATEGORIES, {
        method: 'PUT',
    ...
    <div className="p-4 md:p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      <div className="mb-6 md:mb-8">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold">FAQ Category Management</h1>
            <p className="text-sm md:text-base text-muted-foreground">
              Manage FAQ categories - create, edit, and organize categories
            </p>
          </div>
          <Link href="/admin/content/faq-categories/create" className="w-full sm:w-auto">
        ...
        <Card>
          <CardHeader className="p-4 md:p-6">
            ...
            <CardDescription className="text-xs md:text-sm">
              Categories are used to organize FAQs on the public page
            </CardDescription>
          </CardHeader>
          <CardContent className="p-0 md:p-6">
            ...
            <div className="flex items-center gap-2 md:gap-4 py-3 px-2 md:px-4 border-b bg-muted/50 font-medium text-xs md:text-sm text-muted-foreground">
              ...
              <div className="flex items-center justify-center w-16">FAQs</div>
              ...
              renderItem={(category) => (
                <div className="flex items-center gap-2 md:gap-4 py-3 md:py-4 px-2 md:px-4 border-b last:border-b-0">
                  ...
                  <div className="flex items-center gap-1 md:gap-2">
                    <Link href={`/admin/content/faq-categories/${category.id}/edit`}>
                    ...
                    <Button
                      ...
                      disabled={category._count.faqs > 0}
                    >
