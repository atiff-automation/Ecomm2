  ...
  if (process.env.NEXT_RUNTIME === 'nodejs') {
    ...
    try {
      ...
      if (cacheResult.success) {
        ...
        console.error('❌ Cache system initialization failed:', cacheResult.errors);
      }
      ...
      if (monitoringResult.success) {
        ...
        console.log(`   - Error Reporting: ${monitoringResult.errorReporting ? '✅' : '❌'}`);
        console.log(`   - Performance: ${monitoringResult.performanceMonitoring ? '✅' : '❌'}`);
        console.log(`   - User Tracking: ${monitoringResult.userTracking ? '✅' : '❌'}`);
        
        if (monitoringResult.errors.length > 0) {
          console.warn('⚠️ Monitoring initialization warnings:', monitoringResult.errors);
        }
      } else {
        console.error('❌ Monitoring system initialization failed:', monitoringResult.errors);
      }
      ...
      console.log(`   - Build ID: ${process.env.NEXT_PUBLIC_BUILD_ID || 'development'}`);

      // Schedule periodic health checks in production
      if (process.env.NODE_ENV === 'production') {
        setInterval(async () => {
          try {
            const { getMonitoringHealth } = await import('@/lib/monitoring/monitoring-init');
