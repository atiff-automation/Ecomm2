...
export const basicInfoSchema = z.object({
  fullName: z.string()
    ...
    .regex(/^[\p{L}\p{M}\s'./()-]+$/u, 'Nama penuh hanya boleh mengandungi huruf, ruang, tanda petik, titik, dan tanda khas'),

  icNumber: z.string()
    .regex(icNumberRegex, 'Format No Kad Pengenalan tidak sah. Gunakan format: 123456-12-1234'),

  phoneNumber: z.string()
    .regex(phoneRegex, 'Format No Telefon tidak sah. Contoh: 012-3456789 atau +6012-3456789'),
  ...
  businessLocation: z.string()
    .max(100, 'Tidak boleh melebihi 100 aksara'),
  ...
  if (data.hasBusinessExp && (!data.businessLocation || data.businessLocation.trim() === '')) {
    ctx.addIssue({
...
const instagramHandleRegex = /^[a-zA-Z0-9][a-zA-Z0-9._]*[a-zA-Z0-9]$|^[a-zA-Z0-9]$/;
const tiktokHandleRegex = /^[a-zA-Z0-9][a-zA-Z0-9._]*[a-zA-Z0-9]$|^[a-zA-Z0-9]$/;
...
export const socialMediaSchema = z.object({
  instagramHandle: z.string()
    ...
    .refine(val => {
      ...
      if (val.includes('..') || val.includes('__') || val.includes('.-') || val.includes('-.')) return false;
      if (val.startsWith('.') || val.startsWith('_') || val.startsWith('-')) return false;
      if (val.endsWith('.') || val.endsWith('_') || val.endsWith('-')) return false;
      return /^[a-zA-Z0-9._]{2,30}$/.test(val);
    }, {
      message: 'Format nama Instagram tidak sah. Gunakan hanya huruf, angka, titik dan garis bawah'
    })
    .optional()
    .or(z.literal('')),
  ...
  tiktokHandle: z.string()
    ...
    .refine(val => {
      ...
      message: 'Format nama TikTok tidak sah. Gunakan hanya huruf, angka, titik dan garis bawah'
    })
    .optional()
    .or(z.literal('')),
...
export const additionalInfoSchema = z.object({
  ...
  jrmProducts: z.string()
    .max(500, 'Tidak boleh melebihi 500 aksara'),
  ...
  if (data.hasJrmExp && (!data.jrmProducts || data.jrmProducts.trim() === '')) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['jrmProducts'],
...
export const reviewSchema = z.object({
  finalAgreement: z.boolean()
    .refine(val => val === true, {
      message: 'Anda mesti bersetuju dengan pengakuan ini untuk menghantar permohonan'
    })
});
