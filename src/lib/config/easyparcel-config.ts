...
export const easyParcelConfig = {
   ...
   */
  timeouts: {
    sandbox: parseInt(process.env.EASYPARCEL_SANDBOX_TIMEOUT || '8000', 10),
    production: parseInt(
      process.env.EASYPARCEL_PRODUCTION_TIMEOUT || '15000',
      10
    ),
    validation: parseInt(
      process.env.EASYPARCEL_VALIDATION_TIMEOUT || '10000',
      10
    ),
  },
   ...
   */
  cache: {
    credentialDuration: parseInt(
      process.env.EASYPARCEL_CREDENTIAL_CACHE_DURATION || '300000'
    ), // 5 minutes
    balanceDuration: parseInt(
      process.env.EASYPARCEL_BALANCE_CACHE_DURATION || '600000'
    ), // 10 minutes
    rateDuration: parseInt(
      process.env.EASYPARCEL_RATE_CACHE_DURATION || '3600000'
    ), // 1 hour
  },
  ...
  business: {
    ...
    maxRetries: parseInt(process.env.EASYPARCEL_MAX_RETRIES || '3', 10),
    retryDelay: parseInt(process.env.EASYPARCEL_RETRY_DELAY || '1000', 10),
  },
...
export function getEasyParcelUrl(isSandbox: boolean): string {
  return isSandbox
    ? easyParcelConfig.urls.sandbox
    : easyParcelConfig.urls.production;
}
...
export function getEasyParcelTimeout(isSandbox: boolean): number {
  return isSandbox
    ? easyParcelConfig.timeouts.sandbox
    : easyParcelConfig.timeouts.production;
}
