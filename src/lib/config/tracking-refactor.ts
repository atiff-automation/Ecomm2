...
export const TRACKING_REFACTOR_CONFIG = {
  ...
  PERFORMANCE: {
    REQUEST_TIMEOUT_MS: parseInt(process.env.TRACKING_REQUEST_TIMEOUT || '10000', 10),
    SLOW_REQUEST_THRESHOLD_MS: parseInt(process.env.TRACKING_SLOW_THRESHOLD || '5000', 10),
    MAX_RETRY_ATTEMPTS: parseInt(process.env.TRACKING_MAX_RETRIES || '3', 10),
  },

  // Security Settings
  SECURITY: {
    MAX_LOGIN_ATTEMPTS: parseInt(process.env.TRACKING_MAX_LOGIN_ATTEMPTS || '5', 10),
    ...
    API_KEY_ROTATION_DAYS: parseInt(process.env.TRACKING_API_KEY_ROTATION || '30', 10),
  },
  ...
  AUTO_REFRESH: {
    ...
    MANUAL_COOLDOWN_MS: parseInt(process.env.TRACKING_MANUAL_COOLDOWN || '30000', 10), // 30 seconds
  },
  ...
  NOTIFICATIONS: {
    ...
    ADMIN_ALERT_THRESHOLD: parseInt(process.env.TRACKING_ADMIN_ALERT_THRESHOLD || '10', 10),
  },
  ...
  ARCHIVE: {
    ...
    BATCH_ARCHIVE_SIZE: parseInt(process.env.TRACKING_ARCHIVE_BATCH || '100', 10),
  }
} as const;
...
export const getUpdateFrequency = (status: string): number => {
  ...
  const mappedCategory = TRACKING_REFACTOR_CONFIG.STATUS_UPDATE_MAPPING[normalizedStatus] || 'IN_TRANSIT';
  return TRACKING_REFACTOR_CONFIG.UPDATE_FREQUENCIES[mappedCategory as keyof typeof TRACKING_REFACTOR_CONFIG.UPDATE_FREQUENCIES];
};
...
export const getJobPriority = (jobType: string): number => {
  ...
  return TRACKING_REFACTOR_CONFIG.JOB_PRIORITIES[upperJobType as keyof typeof TRACKING_REFACTOR_CONFIG.JOB_PRIORITIES] || 
...
export const calculateNextUpdate = (
  ...
  frequencyMinutes = Math.max(minFrequency, Math.min(maxFrequency, frequencyMinutes));
...
export const isDeliveryDay = (estimatedDelivery: Date): boolean => {
  ...
  const deliveryDate = new Date(estimatedDelivery.getFullYear(), estimatedDelivery.getMonth(), estimatedDelivery.getDate());
  
  return deliveryDate.getTime() === today.getTime() || deliveryDate.getTime() === tomorrow.getTime();
};
...
export const isWeekendOrHoliday = (): boolean => {
  ...
  // Malaysia Day
  if (month === 9 && date === 16) return true;
  
  // Independence Day (Merdeka)
  if (month === 8 && date === 31) return true;
...
export const getEnvironmentConfig = () => {
  ...
  return {
    ...
    apiTimeout: isDevelopment ? 30000 : TRACKING_REFACTOR_CONFIG.API_MANAGEMENT.REQUEST_TIMEOUT,
    batchSize: isDevelopment ? 5 : TRACKING_REFACTOR_CONFIG.JOB_PROCESSING.BATCH_SIZE,
  };
};
...
export const validateConfiguration = (): { isValid: boolean; errors: string[] } => {
  ...
  const numericConfigs = {
    'UPDATE_FREQUENCIES.IN_TRANSIT': TRACKING_REFACTOR_CONFIG.UPDATE_FREQUENCIES.IN_TRANSIT,
    'JOB_PROCESSING.BATCH_SIZE': TRACKING_REFACTOR_CONFIG.JOB_PROCESSING.BATCH_SIZE,
    'API_MANAGEMENT.REQUEST_TIMEOUT': TRACKING_REFACTOR_CONFIG.API_MANAGEMENT.REQUEST_TIMEOUT,
  };
  ...
  if (TRACKING_REFACTOR_CONFIG.JOB_PROCESSING.BATCH_SIZE > 50) {
    errors.push('JOB_PROCESSING.BATCH_SIZE should not exceed 50 for performance reasons');
  }
