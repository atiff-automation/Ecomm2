...
export function RateLimit(
  ...
  return function (
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      // Increment count
      await cacheManager.set(rateLimitKey, current + 1, {
...
export class CacheWarmer {
  ...
  static async warmUp(taskNames?: string[]): Promise<void> {
    ...
    console.log(
      `✅ Cache warming completed: ${succeeded} succeeded, ${failed} failed`
    );
    ...
    if (failed > 0) {
      results.forEach((result, index) => {
        if (result.status === 'rejected') {
          console.error(
            `❌ Cache warming task ${index} failed:`,
            result.reason
          );
        }
      });
