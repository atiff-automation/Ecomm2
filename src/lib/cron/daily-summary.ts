...
class DailySummaryCron {
  ...
  private async runDailySummary(): Promise<void> {
    try {
      ...
      if (success) {
        console.log(`✅ Daily summary sent successfully for ${yesterday.toDateString()}`);
      } else {
        console.error(`❌ Failed to send daily summary for ${yesterday.toDateString()}`);
      }
    } catch (error) {
  ...
  public async triggerManual(date?: Date): Promise<boolean> {
    try {
      ...
      console.log(`📊 Manually triggering daily summary for ${targetDate.toDateString()}...`);
