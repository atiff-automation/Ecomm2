...
export class PaymentRouterService {
    ...
    try {
      ...
      try {
        ...
        if (apiKey) {
          ...
          results.billplz.error = 'Billplz API key not configured in environment';
        }
        
        if (!results.billplz.configured) {
          results.billplz.error = results.billplz.error || 'Billplz not configured';
        }
      } catch (error) {
        results.billplz.configured = false;
        results.billplz.available = false;
        results.billplz.error = error instanceof Error ? error.message : 'Unknown error';
      }

      // Check toyyibPay availability
      try {
        results.toyyibpay.configured = await toyyibPayService.isServiceConfigured();
        ...
        results.toyyibpay.error = error instanceof Error ? error.message : 'Unknown error';
      }
  ...
  async createPayment(request: PaymentRequest): Promise<PaymentResult> {
    try {
      ...
      console.log(`💰 Amount: RM ${request.amount}, Method: ${request.paymentMethod || 'AUTO'}`);
      ...
      if (!availability[methodKey]?.available) {
        // Try fallback
        const fallbackMethod = paymentMethod === 'TOYYIBPAY' ? 'BILLPLZ' : 'TOYYIBPAY';
        const fallbackKey = fallbackMethod.toLowerCase() as 'billplz' | 'toyyibpay';
        
        if (availability[fallbackKey]?.available) {
          console.log(`⚠️ ${paymentMethod} unavailable, falling back to ${fallbackMethod}`);
   ...
   */
  private async createToyyibPayPayment(request: PaymentRequest): Promise<PaymentResult> {
    try {
   ...
   */
  private async createBillplzPayment(request: PaymentRequest): Promise<PaymentResult> {
    try {
   ...
   */
  async checkPaymentStatus(paymentMethod: PaymentMethod, identifier: string): Promise<{
    success: boolean;
    ...
    try {
      if (paymentMethod === 'TOYYIBPAY') {
        ...
        if (result.success && result.bill) {
          const status = result.bill.paid ? 'paid' 
                       : result.bill.state === 'deleted' ? 'failed' 
                       : 'pending';
