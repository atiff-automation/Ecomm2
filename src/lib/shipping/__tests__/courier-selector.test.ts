...
import {
  CourierSelector,
  type CourierSelectionCriteria,
  type CourierSelectionResult,
...
describe('Courier Selector Algorithm', () => {
  ...
  describe('Courier Selection Logic', () => {
    test('should select best courier based on scoring algorithm', async () => {
      const result = await courierSelector.selectCourier(
        mockRates,
        baseCriteria
      );
  ...
  describe('Geographic Considerations', () => {
    test('should handle West Malaysia destinations', async () => {
      ...
      expect(result).toBeDefined();
      expect(result.selectedRate.estimated_delivery_days).toBeLessThanOrEqual(
        3
      );
    });
  ...
  describe('Automatic Selection for Checkout', () => {
    test('should provide checkout-ready information', async () => {
      const checkoutInfo = await courierSelector.getAutomaticSelection(
        mockRates,
        baseCriteria
      );
  ...
  describe('Validation and Error Handling', () => {
    test('should validate courier selection against business policies', async () => {
      ...
      const validation = await courierSelector.validateSelection(
        result.selectedRate,
        baseCriteria
      );
    ...
    test('should handle empty rates gracefully', async () => {
      await expect(
        courierSelector.selectCourier([], baseCriteria)
      ).rejects.toThrow();
    });
    ...
    test('should handle oversized parcel validation', async () => {
      ...
      expect(validation.errors.some(error => error.includes('weight'))).toBe(
        true
      );
    });
  });
  ...
  describe('Analytics and Reporting', () => {
    test('should provide shipping analytics', async () => {
      ...
      expect(analytics.mostExpensivePrice).toBeGreaterThanOrEqual(
        analytics.cheapestPrice
      );
