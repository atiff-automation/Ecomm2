...
export class EasyParcelCSVExporter {
  ...
  async exportOrdersToCSV(
    ...
    if (!this.businessProfile) {
      throw new Error(
        'Business profile not configured. Please set up business information first.'
      );
    }
   ...
   */
  private async transformOrderToCSVRow(
    order: OrderForExport,
    rowNumber: number
  ): Promise<EasyParcelCSVRow> {
    if (!this.businessProfile) {
    ...
    const customerPhone =
      order.user?.phone || order.guestPhone || '+60123456789'; // Default phone for testing
    ...
    const pickupState = this.mapToEasyParcelState(
      this.businessProfile.pickupAddress.state
    );
    ...
    const csvRow: EasyParcelCSVRow = {
      ...
      receiver_address: this.buildFullAddress(
  ...
  private calculateParcelDetails(orderItems: OrderForExport['orderItems']) {
    ...
    const maxDimensions = { length: 0, width: 0, height: 0 };
   ...
   * Get EasyParcel category from order items
   */
  private getOrderCategory(
    orderItems: OrderForExport['orderItems']
  ): EasyParcelCategory {
   ...
   */
  private getEasyParcelCourierName(
    selectedCourierId?: string
  ): EasyParcelCourier {
    ...
    if (this.businessProfile?.courierPreferences.preferredCouriers.length) {
      const preferredCourier =
        this.businessProfile.courierPreferences.preferredCouriers[0];
  ...
  private getAlternativeCourier(): EasyParcelCourier | undefined {
    if (this.businessProfile?.courierPreferences.preferredCouriers.length > 1) {
      const alternateCourier =
        this.businessProfile.courierPreferences.preferredCouriers[1];
  ...
  private getCODAmount(order: OrderForExport): number | undefined {
    if (
      ...
      this.businessProfile?.serviceSettings.codEnabled
    ) {
  ...
  private validateCSVRow(
    ...
    if (!row.sender_name) {
      errors.push(`${orderRef}: Sender name is required`);
    }
    if (!row.sender_contact) {
      errors.push(`${orderRef}: Sender mobile is required`);
    }
    if (!row.sender_address) {
      errors.push(`${orderRef}: Sender address is required`);
    }
    if (!row.sender_city) {
      errors.push(`${orderRef}: Sender city is required`);
    }
    if (!row.sender_postcode) {
      errors.push(`${orderRef}: Sender postal code is required`);
    }

    // Required receiver fields
    if (!row.receiver_name) {
      errors.push(`${orderRef}: Receiver name is required`);
    }
    if (!row.receiver_contact) {
      errors.push(`${orderRef}: Receiver mobile is required`);
    }
    if (!row.receiver_address) {
      errors.push(`${orderRef}: Receiver address is required`);
    }
    if (!row.receiver_city) {
      errors.push(`${orderRef}: Receiver city is required`);
    }
    if (!row.receiver_postcode) {
      errors.push(`${orderRef}: Receiver postal code is required`);
    }

    // Required parcel fields
    if (!row.weight || row.weight <= 0) {
      errors.push(`${orderRef}: Parcel weight is required and must be > 0`);
    }
    if (!row.parcel_content) {
      errors.push(`${orderRef}: Parcel content description is required`);
    }
    if (!row.parcel_value || row.parcel_value <= 0) {
      errors.push(`${orderRef}: Parcel value is required and must be > 0`);
    }
    if (!row.reference) {
      errors.push(`${orderRef}: Reference number is required`);
    }
    ...
    if (
      row.sender_contact &&
      !phoneRegex.test(row.sender_contact.replace(/[\s-]/g, ''))
    ) {
      errors.push(`${orderRef}: Invalid sender phone format`);
    }
    if (
      row.receiver_contact &&
      !phoneRegex.test(row.receiver_contact.replace(/[\s-]/g, ''))
    ) {
   ...
   */
  private generateCSVString(
    rows: EasyParcelCSVRow[],
    ...
    const headerMapping = [
      ...
      'receiver_city',
      'courier_company',
      'alternative_courier_company',
      'tracking_sms',
      'drop_off_at_courier_branch',
      'reference',
      'tracking_whatsapp',
    ];
