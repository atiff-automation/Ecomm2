 ...
 */
export const ShippingSettingsValidationSchema = z
  .object({
    // API Configuration
    apiKey: z
      .string()
  ...
  .refine(
    ...
    {
      message:
        'At least one courier must be selected when using "Selected Couriers" mode',
  ...
  .refine(
    ...
    {
      message:
        '1st priority courier is required when using "Priority Courier" mode',
  ...
  .refine(
    data => {
      if (
        data.courierSelectionMode === COURIER_SELECTION_STRATEGIES.PRIORITY &&
        data.priorityCouriers
      ) {
    ...
    {
      message:
        'Priority couriers must be unique (cannot select the same courier multiple times)',
 ...
 */
export type ShippingSettingsFormData = z.infer<
  typeof ShippingSettingsValidationSchema
>;
