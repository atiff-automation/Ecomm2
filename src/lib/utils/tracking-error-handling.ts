...
import { TRACKING_REFACTOR_CONFIG, isDebugMode } from '../config/tracking-refactor';
...
export const logTrackingSecurityEvent = (event: Omit<TrackingSecurityLog, 'timestamp'>): void => {
 ...
 */
export const getRecentTrackingSecurityLogs = (limit: number = 100): TrackingSecurityLog[] => {
 ...
 */
export const createTrackingErrorResponse = (error: Error, request?: Request): NextResponse => {
  // Log the error
...
export const trackTrackingAPIPerformance = (
  ...
  const slowThreshold = TRACKING_REFACTOR_CONFIG.PERFORMANCE.SLOW_REQUEST_THRESHOLD_MS;
  if (duration > slowThreshold) {
    console.warn(`🐌 Slow tracking operation detected: ${operation} took ${duration}ms`);
  }
  
  // Alert on failures
  if (!success) {
    console.error(`❌ Tracking operation failed: ${operation} - ${error?.message}`);
  }
};
...
export const getTrackingPerformanceMetrics = (hours: number = 1) => {
  ...
  Object.keys(operations).forEach(op => {
    ...
    opData.averageResponseTime = Math.round(opData.totalDuration / opData.count);
 ...
 */
export const performTrackingSystemHealthCheck = async (): Promise<SystemHealthCheck> => {
  ...
  try {
    // Database check
    try {
      const { getCacheStatistics } = await import('../services/tracking-cache');
      ...
      if (stats.caches.requiresAttention > 20) {
        alerts.push(`Many caches require attention: ${stats.caches.requiresAttention}`);
    ...
    // Job queue check
    try {
      const { trackingJobProcessor } = await import('../jobs/tracking-job-processor');
      const processorStatus = trackingJobProcessor.getStatus();
      checks.jobQueue = true;
      
      // Check if processor is stuck
      if (processorStatus.isProcessing && processorStatus.uptime > 10 * 60 * 1000) {
    ...
    try {
      const { validateCacheConsistency } = await import('../services/tracking-cache');
...
export const attemptTrackingErrorRecovery = async (
  ...
  try {
    if (error instanceof CacheConsistencyError) {
      ...
      if (context.orderId) {
        console.log(`🔄 Attempting cache recovery for order ${context.orderId}`);
